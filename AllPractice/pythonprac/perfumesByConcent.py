...
for i in range(1, len(filters) + 1):
    concentrationName = driver.find_element(By.CSS_SELECTOR,
                                            f'#content > div > section > div.srchResultArea > section.srchFilterArea > ul.srchFilterAccordion > li:nth-child(6) > div > ul > li:nth-child({i}) > label > span > span').text
    driver.find_element(By.CSS_SELECTOR,
                        f'#content > div > section > div.srchResultArea > section.srchFilterArea > ul.srchFilterAccordion > li:nth-child(6) > div > ul > li:nth-child({i}) > label > input').click()
    ...
    for a in range(pagesCnt):
        ...
        for li in uls:
            ...
            if a is not None:
                ...
                # 해당 단어가 없는 경우에만 진행
                if title.find('세트') == -1 and title.find('SET') == -1 and title.find('컬렉션') == -1 and title.find('셀렉션') == -1:
                    ...
                    if len(title) > 1:
                        ...
                        title = title[0]
                    print(f'[블라블라] 제거 : {title}')

                    # ML 부터 내용 날려버리기
                    if title.find('ML') != -1:
                        a = title.index('ML') + 2
                        title = title[0:a]
                        print(f'ML 뒤에 날리기 : {title}')
...
df.to_csv('./perfumesByConcent.cvs', index=False)
