...
export class ProductsService {
  ...
  async create({
    ...
    // 2-2. 상품태그 등록
    const tags = await this.productsTagsService.insertByProductTags({
      productTags,
    });
  ...
  async update({
    ...
    const { productTags, productCategoryId, ...updateProduct } =
      updateProductInput;

    const tags = productTags
      ? await this.productsTagsService.insertByProductTags({ productTags })
      : product.productTags;
    ...
    const result = this.productsRepository.save({
      ...
      ...updateProduct,
      productCategory: {
        id: productCategoryId || product.productCategory.id,
      },
      productTags: tags,
    });
